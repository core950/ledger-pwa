...
- 商品核算：填写来源、价格、规格和份数，自动计算每单位/每份价格，并可填入每日分类
- 导入账单保留到秒，流水按具体时间排序
...
`核算` 页可以单独保存商品来源、总价、规格数量、规格单位和份数。应用会自动算出：

- 每规格单位价格，例如每 `g`、每 `个`、每 `斤`
- 每份价格，由你填写“分成几份”后计算

点击 `填入每日` 会生成一条每日账单明细，分类使用商品核算里填写的分类，金额使用每份价格。这条明细只用于每日分类记录，不计入每日/本月收入支出总额。

商品核算记录会复用 Supabase 的 `ledger_records` 表同步，云端 `source` 会显示为 `商品核算`，不需要额外建表。
